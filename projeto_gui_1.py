...
def cadastrar_conta_corrente():
  ...
  saldos_iniciais.append(saldo_inicial)
  limite_credito = validar_entrada_numerica_cadastro_conta_corrente('LIMITE DE CRÉDITO: ', 0)
  limites_credito.append(limite_credito)
  senha_conta = validar_entrada_texto_cadastro_conta_corrente('SENHA: ')
  senhas.append(senha_conta)
  repita_senha_conta = validar_entrada_texto_cadastro_conta_corrente('REPITA SUA SENHA: ')

  while repita_senha_conta != senha_conta:
...
def depositar():
  ...
  senha = input("Senha: ")
  if conta == contas[0] and senha == senhas[0]:
    valor_deposito = int(input("Valor do Depósito: "))
    saldo_final += saldos_iniciais[0] + valor_deposito
  print(f"O seu saldo atual é de R$ {saldo_final:.2f}")
  input('Pressione ENTER para continuar...')
